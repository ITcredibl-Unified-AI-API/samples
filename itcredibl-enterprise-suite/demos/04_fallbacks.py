...
FALLBACKS = [
    p.strip()
    for p in os.getenv("ITCREDIBL_FALLBACKS", "openai,anthropic,groq").split(",")
    if p.strip()
]
        ...
        try:
            r = await c.chat(
                messages=[
                    {
                        "role": "user",
                        "content": "One sentence: why fallbacks protect SLAs.",
                    }
                ],
                model=MODEL,
                provider=provider,
            ...
            text = (r.get("choices", [{}])[0].get("message", {}) or {}).get(
                "content"
            ) or ""
            usage = r.get("usage", {})
            kv_table(
            ...
            last_error = e
            kv_table(
                {"provider": provider, "ok": False, "error": str(e)},
                title=f"Failed: {provider}",
            )
        finally:
            await c.aclose()
