...
from itcredibl_enterprise.console import header, box, kv_table
...
async def main():
    ...
    try:
        u = await c.usage()
        txt = str(u)
        box((txt[:1200] + "…") if len(txt) > 1200 else txt, title="Gateway Usage Response")
        await c.aclose()
        return
    except ITCError:
        pass
    ...
    total_tokens = sum(int(u.get("total_tokens") or ((u.get("prompt_tokens") or 0) + (u.get("completion_tokens") or 0))) for u in usages)
