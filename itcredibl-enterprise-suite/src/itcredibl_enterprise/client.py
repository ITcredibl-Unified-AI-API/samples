...
DEFAULT_BASE_URL = (
    os.getenv("ITCREDIBL_API_URL") or "https://api.itcredibl.com"
).rstrip("/")
...
def _classify(status: int) -> ITCError:
    if status in (401, 403):
        return ITCAuthError("Auth failed or not authorized")
    if status == 429:
        return ITCRateLimitError("Rate limit exceeded")
    if 500 <= status < 600:
        return ITCServerError(f"Server error {status}")
    return ITCError(f"HTTP {status}")


def _headers(api_key: str, org: str | None) -> dict[str, str]:
    h = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
        "User-Agent": "itcredibl-enterprise-demo/1.0",
    }
    if org:
...
class ITcrediblClient:
    ...
    def _is_function_base(self) -> bool:
        return "/functions/" in self.base_url or self.base_url.endswith(
            "/itcredibl-api"
        )
    ...
    @retry(
        ...
        retry=retry_if_exception_type(
            (ITCServerError, ITCRateLimitError, httpx.TransportError)
        ),
    )
    async def chat(
        self,
        ...
        **kwargs: Any,
        ...
        resp = await self._client.post(
            self._route("/v1/chat/completions"), json=payload
        )
        if resp.status_code != 200:
            raise _classify(resp.status_code)
        data = resp.json()
        self._emit_metrics("chat", t0, data, payload)
        return data
    ...
    async def chat_stream(
        ...
        **kwargs: Any,
        ...
        async with self._client.stream(
            "POST",
            self._route("/v1/chat/completions"),
            json=payload,
    ...
    async def tool_call(
        ...
        **kwargs: Any,
        ...
        if resp.status_code != 200:
            raise _classify(resp.status_code)
        data = resp.json()
        self._emit_metrics("tool_call", t0, data, payload)
        return data
    ...
    async def embeddings(
        ...
        t0 = time.perf_counter()
        resp = await self._client.post(self._route("/v1/embeddings"), json=payload)
        if resp.status_code != 200:
            raise _classify(resp.status_code)
        data = resp.json()
        self._emit_metrics("embeddings", t0, data, payload)
        return data

    async def moderate(self, text: str) -> dict[str, Any]:
        payload = {"input": text}
        t0 = time.perf_counter()
        resp = await self._client.post(self._route("/v1/moderations"), json=payload)
        if resp.status_code != 200:
            raise _classify(resp.status_code)
        data = resp.json()
        self._emit_metrics("moderation", t0, data, payload)
        return data
    ...
    async def usage(
        ...
        t0 = time.perf_counter()
        resp = await self._client.get(self._route("/v1/usage"), params=params)
        if resp.status_code != 200:
            raise _classify(resp.status_code)
        data = resp.json()
        self._emit_metrics("usage", t0, data, {"params": params})
        return data

    def _emit_metrics(
        self, op: str, t0: float, data: dict[str, Any] | None, meta: dict[str, Any]
    ):
